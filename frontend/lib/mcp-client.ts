...
import {
  ...
  LoggingMessageNotificationSchema, // ロギングメッセージ通知のスキーマ定義
...
export async function initializeClient() {
  ...
  try {
    ...
    client = new Client({
      name: "mcp-streaming-chat-frontend", // クライアントの名前を指定 - サーバー側でクライアントを識別するために使用
...
/**
 * チャットメッセージを送信する非同期関数 - サーバー上のchatツールを呼び出してレスポンスを取得
 * @param message ユーザーからのメッセージ
 * @param onChunkReceived 各チャンクを受信したときのコールバック関数（オプション）
 * @returns チャットレスポンス（文字列）
 * @throws メッセージ送信中にエラーが発生した場合
 */
export async function sendChatMessage(
  message: string,
  onChunkReceived?: (text: string, metadata: any) => void,
) {
  // メッセージのバリデーション - 空のメッセージをチェック
  if (!message.trim()) {
    throw new Error("Message cannot be empty"); // 空のメッセージの場合はエラーをスロー
  }
  ...
  try {
    console.log("===== 非ストリーミングチャット開始 =====");
    console.log("リクエスト:", message);

    // コールバック関数が提供されている場合は通知ハンドラを設定
    mcpClient.setNotificationHandler(
      ...
      (notification) => {
        ...
        // デバッグ用にコンソールに出力
        console.log("===== 非ストリーミング通知受信 =====");
        console.log("通知データ:", text);
        console.log("メタデータ:", metadata);
        console.log("通知オブジェクト:", notification);
    ...
    const req: CallToolRequest = {
      method: "tools/call", // ツール呼び出しメソッドを指定 - MCP仕様で定義されたメソッド名
      params: {
        name: "chat", // 呼び出すツールの名前を指定 - サーバー側で登録されているツール名
        arguments: { message }, // ツールに渡す引数を指定 - ユーザーメッセージ
      },
    };
    ...
    const textContent = res.content.find((item) => item.type === "text");
    if (textContent) {
      // コールバック関数が提供されている場合は最終結果も通知
      if (onChunkReceived && textContent.text) {
        onChunkReceived(textContent.text, textContent.metadata || {});
      }
      return textContent.text; // テキストコンテンツが見つかった場合はその値を返す
    } else {
      throw new Error("No text content in response"); // テキストコンテンツが見つからない場合はエラー
    ...
    // エラーが発生した場合
    console.error("Error sending chat message:", error); // エラー情報をログに出力
    throw error; // エラーを呼び出し元に伝播させる
  }
}

/**
 * ストリーミングチャットメッセージを送信する非同期関数 - サーバー上のchat_streamツールを呼び出してレスポンスを取得
 * @param message ユーザーからのメッセージ
 * @param onChunkReceived 各チャンクを受信したときのコールバック関数
 * @returns ツール呼び出しの結果
 * @throws メッセージ送信中にエラーが発生した場合
 */
export async function sendStreamingChatMessage(
  ...
  try {
    console.log("===== ストリーミングチャット開始 =====");
    console.log("リクエスト:", message);

    // 通知ハンドラを設定 - ストリーミングチャンクを受信するためのハンドラ
    mcpClient.setNotificationHandler(
      LoggingMessageNotificationSchema,
      (notification) => {
        // 通知からテキストとメタデータを取得
    ...
    const req: CallToolRequest = {
      method: "tools/call", // ツール呼び出しメソッドを指定 - MCP仕様で定義されたメソッド名
      params: {
        name: "chat_stream", // 呼び出すツールの名前を指定 - サーバー側で登録されているツール名
        arguments: { message }, // メッセージのみを引数として渡す
      },
    };
    ...
    // レスポンスのコンテンツをチェック
    // 最初のチャンクが通知として送信される場合、ここでは何もしない
    const textContent = res.content.find((item) => item.type === "text");
    if (textContent && textContent.text) {
      // テキストが存在する場合のみコールバックを呼び出す
      onChunkReceived(textContent.text, textContent.metadata || {});
    }
    ...
    // エラーが発生した場合
    console.error("Error sending streaming chat message:", error); // エラー情報をログに出力
    throw error; // エラーを呼び出し元に伝播させる
  }
}
